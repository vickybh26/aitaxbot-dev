...
export default function BlogPost() {
    ...
    return (
      <div className="flex items-center justify-center bg-secondary">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-ink mb-4">Post Not Found</h1>
          <p className="text-ink/65 mb-8">The blog post you're looking for doesn't exist.</p>
          <Link href="/blog">
    ...
    return (
      <>
        {parts.map((part, i) => {
          ...
          if (part.startsWith('`') && part.endsWith('`'))
            return <code key={i} className="bg-secondary rounded px-1 font-mono text-sm">{part.slice(1, -1)}</code>;
  ...
  const renderTable = (tableLines: string[], keyBase: number) => {
    ...
    return (
      <div key={`tbl-${keyBase}`} className="overflow-x-auto my-6 rounded-lg border border-rule shadow-sm">
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri} className={`border-b border-rule last:border-b-0 transition-colors hover:bg-secondary ${ri % 2 === 0 ? 'bg-card' : 'bg-secondary'}`}>
                {row.map((cell, ci) => (
                  <td key={ci} className="px-4 py-3 text-ink/80 border-r border-rule last:border-r-0">
                    {renderInline(cell)}
                  </td>
                ))}
              </tr>
  ...
  const renderMarkdown = (content: string) => {
    ...
    while (i < lines.length) {
      ...
      // Horizontal rule
      if (trimmed === '---') {
        nodes.push(<hr key={i} className="border-rule my-6" />);
        i++; continue;
      }
      ...
      <div className="bg-card">
        ...
        <div className="bg-card border-b">
          <div className="max-w-4xl mx-auto px-6 py-4">
            <div className="flex items-center gap-2 text-sm text-ink/65">
              <Link href="/blog" className="hover:text-credit">Blog</Link>
        ...
        <article className="max-w-4xl mx-auto px-6 py-12">
          <div className="mb-8">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-ink mb-6 leading-tight" data-testid="post-title">
              {post.metaTitle}
            </h1>
            
            <div className="flex items-center gap-6 text-ink/65 mb-3 flex-wrap">
              ...
              <button
                onClick={handleShare}
                className="flex items-center gap-2 text-credit hover:text-credit"
            ...
            <div className="flex items-center gap-3 bg-secondary border border-rule rounded-lg px-4 py-3 mb-6">
          ...
          <div className="prose prose-lg max-w-none">
            {post.bodySections.map((section, index) => {
              ...
              if (section.type === 'intro' || section.type === 'outro') {
                sectionContent = (
                  <div key={index} className="mb-8 text-lg leading-relaxed text-ink/80">
              ...
              if (section.type === 'h2') {
                sectionContent = (
                  <div key={index} className="mb-8">
                    <h2 className="text-3xl font-bold text-ink mb-4 mt-12" data-testid={`heading-${index}`}>
              ...
              if (section.type === 'h3') {
                sectionContent = (
                  <div key={index} className="mb-6">
                    <h3 className="text-2xl font-bold text-ink mb-3 mt-8" data-testid={`subheading-${index}`}>
                      {section.heading || section.title}
                    </h3>
                    {section.content_md && (
                      <div className="text-ink/80 leading-relaxed">
              ...
              if (section.type === 'faq') {
                sectionContent = (
                  <Card key={index} className="p-6 mb-8 bg-secondary border-rule" data-testid="faq-section">
                    <h3 className="text-2xl font-bold text-ink mb-6">Frequently Asked Questions</h3>
                    <div className="space-y-4">
                      {section.items?.map((faq: { q: string; a: string }, faqIdx: number) => (
                        <div key={faqIdx} className="border-b border-rule last:border-b-0 pb-4 last:pb-0">
                          <p className="font-semibold text-ink mb-2" data-testid={`faq-question-${faqIdx}`}>
                            Q: {faq.q}
                          </p>
                          <p className="text-ink/80" data-testid={`faq-answer-${faqIdx}`}>
                            A: {faq.a}
                          </p>
                        </div>
                      ))}
                      ...
                      <div className="flex flex-wrap gap-3">
                        {section.internal_links.map((link: { href: string; label: string }, linkIdx: number) => (
                          <Link key={linkIdx} href={link.href}>
                            <Button
                              variant="secondary"
                              className="bg-card text-credit hover:bg-secondary"
          ...
          <div className="mt-12 bg-secondary border border-rule rounded-xl p-5">
            <div className="flex items-start gap-3">
              <ShieldCheck className="h-5 w-5 text-ink/55 mt-0.5 shrink-0" />
              <div>
          ...
          {post.relatedPosts && post.relatedPosts.length > 0 && (
            <div className="mt-12 pt-8 border-t" data-testid="related-posts">
              <h3 className="text-xl font-bold text-ink mb-4">Related Articles</h3>
              <div className="grid gap-3 sm:grid-cols-3">
                {post.relatedPosts.map((related) => (
                  <Link key={related.slug} href={`/blog/${related.slug}`}>
                    <div className="group flex items-start gap-3 p-4 rounded-lg border border-rule hover:border-blue-400 hover:bg-secondary transition-colors cursor-pointer">
                      <ChevronRight className="h-4 w-4 text-blue-500 mt-0.5 shrink-0 group-hover:translate-x-0.5 transition-transform" />
                      <span className="text-sm font-medium text-ink/80 group-hover:text-credit leading-snug">
                        {related.title}
                      </span>
                    </div>
                  </Link>
