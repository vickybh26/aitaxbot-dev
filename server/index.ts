...
// ── No scheduled email ────────────────────────────────────────────────────────
// There was a cron here until 2026-09-07: `cron.schedule("0 9 * * 1", ...)`,
// which sent the weekly digest to every registered user each Monday at 9am
// IST with nobody having read it first. It was removed on the founder's
// instruction after the 2026-09-07 run, and the reason is worth keeping:
// the problems in that send — an unlabelled ₹1.59 crore PF projection
// rendered beside a ₹3,51,000 tax bill as though they were comparable — were
// only found by reading the copy that arrived in his own inbox, hours after
// 146 other people had already received it.
//
// The digest is now monthly, written per issue, and sent by hand from
// /admin/digest. See server/monthlyDigest.ts. Do not reintroduce a schedule
// here without asking: welcome and calculator-result emails still fire
// automatically, and those are the ones that should, because each is a 1:1
// reply to something the user just did rather than a broadcast.
//
// cleanStalePdfs() above still runs once at boot; there is no recurring job
